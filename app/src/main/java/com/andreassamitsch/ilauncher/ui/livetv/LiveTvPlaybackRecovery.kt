 ...
 * target Gigablue/TCL setup these parser errors are observed during rapid zapping, while manually
 * reconnecting to the same service succeeds. Retry only those parser failures and keep the number
 * of full stream restarts small so a genuinely unsupported stream still surfaces as an error.
 */
