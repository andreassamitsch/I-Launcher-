...
Die aktuelle Filme-/Serien-Discovery besitzt zusätzlich Google-TV-artige „Mehr“-Unterseiten, globale Anime-/Kindermodus-Filter und launcher-eigene Genre-Relevanz. Film-Discover verwendet `language=de-DE` für lokalisierte Metadaten und `region=DE` für deutsche Veröffentlichungsdaten; `region` ist ausdrücklich kein Sprach-/Dub-Filter. Bei sichtbar unveränderten fremdsprachigen Originaltiteln außerhalb Deutsch/Englisch prüft I Launcher deshalb gezielt TMDBs `/movie/{id}/translations` und behält den Film nur, wenn eine deutsche Titelübersetzung hinterlegt ist. Bereits durch `de-DE` eindeutig lokalisierte Titel sowie deutsche/englische Originaltitel benötigen diesen Zusatzaufruf nicht. Die Translation-Ergebnisse werden begrenzt parallel geladen und sieben Tage im Prozesscache gehalten; der Check belegt deutsche TMDB-Metadaten, nicht das Vorhandensein einer deutschen Tonspur.

Für die kuratierte Kategorie Komödie werden Titel mit mindestens zwei stark dominierenden Fremdgenres (z. B. Adventure + Sci-Fi, Action + Adventure oder Crime + Thriller) verworfen, weil TMDB `with_genres` keinen Hauptgenre-Begriff besitzt. Genre-Unterseiten „Top bewertet“ verlangen bei Filmen nun mindestens 500 Stimmen, damit kleine neue Titel mit sehr hoher Durchschnittswertung etablierte Filme nicht überholen. Unterseiten tragen die jeweilige Ausgangskategorie in jeder Reihenüberschrift.
